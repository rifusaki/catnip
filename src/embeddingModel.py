...
def build_model(IMG_SIZE: int, CROPS_DIR: str | os.PathLike, load_weights: bool = True):
    ...
    if load_weights:
        model_weights_path = settings.paths.embs_dir.parent / "model.weights.h5"
...
def compute_embeddings(embed_model, CROPS_DIR: str | os.PathLike, IMG_SIZE: int):
    ...
    np.save(settings.paths.embs_dir, embs)
    with open(settings.paths.crops_dir, "w") as f:
        json.dump(valid_paths, f)
    
    # Save model weights for consistency across sessions
    model_weights_path = settings.paths.model_dir / "model.weights.h5"
    embed_model.save_weights(str(model_weights_path))

    print(f"Saved embeddings to {settings.paths.embs_dir}")
    print(f"Saved crop paths to {settings.paths.crops_dir}")
    print(f"Saved model weights to {model_weights_path}")

    return embs, valid_paths


def load_embeddings(EMBED_PATH = settings.paths.embs_dir, CROP_PATH = settings.paths.crops_dir):
    """
